// A clone of  https://github.com/jackc/pgx/blob/master/stdlib/sql.go that connects
// to DSQL instead of Postgres directly.
...
func (dc *driverConnector) Connect(ctx context.Context) (driver.Conn, error) {
	...
	if connConfig == nil {
		var err error
		connConfig, err = dc.driver.dsqlConnect(dc.name)
		if err != nil {
			return nil, err
		}
	}
...
func (d *Driver) dsqlConnect(connStr string) (*pgx.ConnConfig, error) {
	...
	connConfig.RuntimeParams["application_name"] = "dsql_driver"

	return connConfig, nil
}

func generateDBConnectAdminAuthToken(creds *credentials.Credentials, endpoint, region string) (string, error) {
